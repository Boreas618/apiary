"""Apiary per-session client and HTTP transport.

:class:`AsyncApiarySession` manages a single sandbox session (async).
:class:`ApiarySession` is its synchronous wrapper.
:class:`ApiarySessionMux` multiplexes many clients over a shared HTTP
connection to the Apiary daemon.

All three share a common :class:`_Transport` for HTTP and exec calls.
``_Transport`` also exposes raw image-registry endpoints used by
:class:`apiary_client.apiary.AsyncApiary` (the canonical client).
"""
...
class RegisterResponse:
    """202-style ack returned from ``POST /api/v1/images``.

    ``queued`` lists images that were not yet registered and have been
    accepted for loading. ``already_present`` lists images that the
    daemon's registry already had — those skip the load pipeline.
    """
...
@dataclass
class ImageProgress:
    """Per-image progress entry inside an :class:`ImageJobStatus`.

    ``state`` is one of:
    ``"queued"`` | ``"pulling"`` | ``"extracting"`` | ``"alreadypresent"``
    | ``"done"`` | ``"failed"``.
    """
    ...
    def is_terminal(self) -> bool:
        return self.state in ("alreadypresent", "done", "failed")

    @property
    def is_success(self) -> bool:
        return self.state in ("alreadypresent", "done")


@dataclass
class ImageJobStatus:
    """Snapshot of an image-load job from
    ...
    def succeeded(self) -> list[str]:
        return [name for name, p in self.per_image.items() if p.is_success]

    @property
    def failed(self) -> list[str]:
        return [name for name, p in self.per_image.items() if p.state == "failed"]


class ImageJobNotFound(LookupError):
    """Raised when ``GET /api/v1/images/jobs/{id}`` returns 404.

    A job id is unknown if it was never submitted, was reaped after the
    job-retention window, or was lost on daemon restart.
    """
...
def _parse_image_progress(data: dict) -> ImageProgress:
    """Parse a per-image progress entry returned by the daemon.

    The daemon's serde-tagged enum lands as a dict with a ``state`` key
    plus the per-variant fields. We map every variant onto a single
    flat :class:`ImageProgress` instance.
    """
    ...
    layers_done = int(data.get("layers_done", 0) or 0)
    layers_total = int(data.get("layers_total", 0) or 0)
    error = data.get("reason") or data.get("error")
...
def _parse_image_job_status(data: dict) -> ImageJobStatus:
    raw_per_image = data.get("per_image") or {}
    per_image: dict[str, ImageProgress] = {
        name: _parse_image_progress(payload)
        for name, payload in raw_per_image.items()
    }
    return ImageJobStatus(
        ...
        per_image=per_image,
        failed_images=list(data.get("failed_images") or []),
    )


def _parse_register_response(data: dict) -> RegisterResponse:
    return RegisterResponse(
        job_id=data["job_id"],
        queued=list(data.get("queued") or []),
        already_present=list(data.get("already_present") or []),
    )
...
class _Transport:
    ...
    Encapsulates HTTP client management, session CRUD, raw command
    execution, and the image-registry endpoints. Shared between
    :class:`AsyncApiarySession`, :class:`ApiarySessionMux`, and the
    canonical :class:`apiary_client.apiary.AsyncApiary` client.
    """
    ...
    async def register_images(
        ...
        """Submit a list of images for async loading.

        Returns the daemon's 202 ack with a ``job_id`` to poll. Images
        already in the registry land in ``already_present`` and bypass
        the load pipeline entirely.
        """

        if not images:
            raise ValueError("register_images: `images` must be non-empty")

        http = await self.http()
        payload = {"images": list(images)}
        resp = await http.post("/api/v1/images", json=payload, timeout=30)
        _raise_for_status(resp, context="register_images", payload=payload)
        return _parse_register_response(resp.json())

    async def image_job(self, job_id: str) -> ImageJobStatus:
        """Fetch a single snapshot of an image-load job.

        Raises :class:`ImageJobNotFound` when the daemon returns 404
        (unknown id, or reaped after the retention window).
        """

        http = await self.http()
        resp = await http.get(f"/api/v1/images/jobs/{job_id}", timeout=15)
        if resp.status_code == 404:
            raise ImageJobNotFound(f"image job not found: {job_id}")
        _raise_for_status(resp, context=f"image_job({job_id})")
        return _parse_image_job_status(resp.json())
    ...
    async def wait_for_image_job(
        ...
        *,
        ...
        """Poll ``image_job(job_id)`` until terminal or *timeout* elapses.

        ``on_progress`` (if given) is called with each non-terminal
        snapshot — useful for driving a progress bar. The final terminal
        snapshot is *not* passed to ``on_progress`` and is returned
        directly so the caller can inspect it.

        Raises :class:`asyncio.TimeoutError` when *timeout* (in seconds)
        elapses without reaching a terminal state.
        """

        if poll <= 0:
            raise ValueError("wait_for_image_job: `poll` must be > 0")

        deadline = None if timeout is None else time.monotonic() + timeout
        while True:
            status = await self.image_job(job_id)
            ...
            if on_progress is not None:
                try:
                    ...
                    logger.warning(
                        "on_progress callback raised; continuing to poll",
                        exc_info=True,
                    )
            if deadline is not None and time.monotonic() >= deadline:
                raise asyncio.TimeoutError(
                    f"image job {job_id} did not finish within {timeout:.1f}s"
                )
            await asyncio.sleep(poll)

    async def list_images(self) -> list[str]:
        """List every image currently registered with the daemon."""

        http = await self.http()
        resp = await http.get("/api/v1/images", timeout=10)
        _raise_for_status(resp, context="list_images")
        data = resp.json()
        return list(data.get("images") or [])

    async def unregister_image(self, name: str) -> None:
        """Remove ``name`` from the registry. 404 if it isn't registered."""

        http = await self.http()
        resp = await http.delete(f"/api/v1/images/{name}", timeout=15)
        _raise_for_status(resp, context=f"unregister_image({name})")
...
class AsyncApiarySession:
    ...
    * File-operation helpers implemented as ``execute()`` wrappers.
    ...
    async def __aenter__(self) -> AsyncApiarySession:
...
class ApiarySession:
    """Synchronous facade over :class:`AsyncApiarySession`.
    ...
    def __init__(
        ...
        *,
        ...
        self._async = AsyncApiarySession(
            image=image,
    ...
    def __enter__(self) -> ApiarySession:
        return self
